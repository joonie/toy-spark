...
code_list = ['071050', '000660', '005380']

appended_data = []
for code in code_list:
    with urllib.request.urlopen('https://finance.naver.com/item/main.nhn?code='+code) as response:
        html = response.read()
    ...
    #print(finance_index) #['매출액', '영업이익', '당기순이익', '영업이익률', '순이익률', 'ROE(지배주주)', '부채비율', '>당좌비율', '유보율', 'EPS(원)', 'PER(배)', 'BPS(원)', 'PBR(배)', '주당배당금(원)', '시가배당률(%)', '배당성향(%)']
    ...
    test_df = pd.DataFrame(quarter_finance, index=['매출액', 'ROE(지배주주)', 'BPS(원)'])
    t_test_df = test_df.transpose()
    t_test_df['code'] = code
    appended_data.append(t_test_df)
